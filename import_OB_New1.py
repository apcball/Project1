...
class FailedRecordQueue:
    def __init__(self, max_retries=3, backup_dir='failed_records'):
        self.queue = Queue()
        self.max_retries = max_retries
        self.backup_dir = Path(backup_dir)
        self.backup_dir.mkdir(exist_ok=True)
        self.lock = threading.Lock()
        self._load_backed_up_records()

    def add(self, record: dict, error_message: str):
        failed_record = FailedRecord(
            record_data=record,
            error_message=error_message
        )
        self.queue.put(failed_record)
        self._backup_record(failed_record)

    def get(self) -> Optional[FailedRecord]:
        try:
            return self.queue.get_nowait()
        except Empty:
            return None

    def _backup_record(self, failed_record: FailedRecord):
        with self.lock:
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            record_id = failed_record.record_data.get('id', 'unknown')
            backup_file = self.backup_dir / f"failed_record_{record_id}_{timestamp}.json"
            
            data = {
                'record_data': failed_record.record_data,
                'error_message': failed_record.error_message,
                'retry_count': failed_record.retry_count,
                'last_retry_time': failed_record.last_retry_time
            }
            
            with open(backup_file, 'w') as f:
                json.dump(data, f, indent=2)

    def _load_backed_up_records(self):
        for file in self.backup_dir.glob('failed_record_*.json'):
            try:
                with open(file, 'r') as f:
                    ...
                    failed_record = FailedRecord(
                        ...
                        last_retry_time=data['last_retry_time']
                    )
                    self.queue.put(failed_record)
            except Exception as e:
                logger.error(f"Error loading backed up record from {file}: {str(e)}")

    def mark_success(self, failed_record: FailedRecord):
        # Remove backup file if exists
        with self.lock:
            for file in self.backup_dir.glob(f"failed_record_{failed_record.record_data.get('id', 'unknown')}*.json"):
                try:
                    file.unlink()
                except Exception as e:
                    logger.error(f"Error removing backup file {file}: {str(e)}")

    def get_stats(self):
        return {
            'queue_size': self.queue.qsize(),
            'backup_files': len(list(self.backup_dir.glob('failed_record_*.json')))
        }
