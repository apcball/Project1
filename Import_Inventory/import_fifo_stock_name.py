...
def read_excel_file():
    ...
    try:
        ...
        # Forward fill name column if it exists to group correctly
        if 'name' in df.columns:
            df['name'] = df['name'].ffill()
            logger.info("Forward filled 'name' column")
...
def create_internal_transfers(uid, models, df):
    ...
    try:
        ...
        if 'scheduled_date' in df.columns:
            sort_cols.append('scheduled_date')
        if 'sequence' in df.columns:
        ...
        group_col = 'name' if has_name_col else 'scheduled_date'

        # Group ตาม name หรือ scheduled_date
        for group_key, group_df in df.groupby(group_col):
            group_df = group_df.reset_index(drop=True)
            first_row = group_df.iloc[0]
            
            scheduled_date = first_row.get('scheduled_date', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            ...
            origin_val = first_row.get('origin')
            origin_str = str(origin_val).strip() if pd.notna(origin_val) and str(origin_val).strip() != 'nan' else f"Import {date_str.split(' ')[0]}"
            ...
            for idx, row in group_df.iterrows():
                try:
                    ...
                    row_seq = row.get('sequence') if 'sequence' in row and pd.notna(row.get('sequence')) else ''
                    
                    move_vals = {
                        'name': f"Move {product_code} | Seq:{row_seq} | Line:{idx+1}",
                        'product_id': product_id,
                        'product_uom_qty': quantity,
                        'product_uom': uom_id,
                        'picking_id': picking_id,
                        'location_id': source_location_id,
                        'sequence': int(row_seq) if row_seq != '' else 10,
                        'description_picking': f"Row {idx+1} | Seq:{row_seq} | Date:{row.get('date', '')}",
                    }
                    ...
                    if 'price_unit' in row and pd.notna(row['price_unit']):
                        try:
                            price_val = float(row['price_unit'])
                            move_vals['price_unit'] = price_val
                            move_vals['custom_cost_price'] = price_val
                        except (ValueError, TypeError):
