...
# Excel file path
EXCEL_FILE = 'Import_Inventory/Template_Internal_Transfer.xlsx'

# Configure logging
log_filename = f'internal_tranfer_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(
    level=logging.INFO,
...
def read_excel_file():
    ...
    try:
        ...
        if 'sequence' in df.columns:
            ...
            df['sequence'] = 0
            logger.info("Added missing sequence column with default 0")
...
def create_internal_transfers(uid, models, df):
    ...
    try:
        ...
        group_col = 'scheduled_date'
        if 'name' in df.columns:
             group_col = 'name'
             logger.info("Grouping by 'name' column")
             # Sort by name and sequence if grouping by name
             df = df.sort_values(by=['name', 'sequence']).reset_index(drop=True)
        else:
             logger.info("Grouping by 'scheduled_date' (fallback)")
             df = df.sort_values(by=['scheduled_date', 'sequence']).reset_index(drop=True)

        # Group by the determined column
        for group_key, group_df in df.groupby(group_col):
            ...
            scheduled_date = first_row.get('scheduled_date')
            ...
            # Use 'name' from Excel if grouping by name
            if group_col == 'name':
                picking_name = str(group_key).strip()
                picking_vals['name'] = picking_name
                
                # Check duplication
                existing_pickings = models.execute_kw(DB, uid, PASSWORD, 
                    'stock.picking', 'search', 
                    [[['name', '=', picking_name]]]
                )
                if existing_pickings:
                    logger.warning(f"Picking '{picking_name}' already exists. Skipping.")
                    continue
