...
def update_operation_bom(uid, models, data):
    ...
    for index, row in data.iterrows():
        try:
            ...
            # Find BOM by code first
            bom_ids = models.execute_kw(DB, uid, PASSWORD,
                'mrp.bom', 'search',
                [[['code', '=', product_code]]]
            )

            # If no BOM by code, try to locate product then BOM by product/template
            if not bom_ids:
                print(f"No BOM found by code for product: {product_code}. Trying product lookup...")

                # Try to find product by default_code
                product_ids = models.execute_kw(DB, uid, PASSWORD,
                    'product.product', 'search',
                    [[['default_code', '=', product_code]]]
                )

                # If not found, try to find by old_product_code (fallback)
                if not product_ids:
                    product_ids = models.execute_kw(DB, uid, PASSWORD,
                        'product.product', 'search',
                ...
                if product_ids:
                    # Read product to get template id
                    product_data = models.execute_kw(DB, uid, PASSWORD,
                        'product.product', 'read', [product_ids[0]], {'fields': ['id', 'product_tmpl_id']}
                    )
                    if product_data:
                        prod = product_data[0]
                        tmpl_id = prod.get('product_tmpl_id')

                        # Try BOM by product template
                        if tmpl_id:
                            bom_ids = models.execute_kw(DB, uid, PASSWORD,
                                'mrp.bom', 'search',
                                [[['product_tmpl_id', '=', tmpl_id[0] if isinstance(tmpl_id, list) else tmpl_id]]]
                            )

                        # If still not found, try BOM by product id
                        if not bom_ids:
                            bom_ids = models.execute_kw(DB, uid, PASSWORD,
                                'mrp.bom', 'search',
                                [[['product_id', '=', prod['id']]]]
...
def main():
    ...
    excel_file = r"Data_file/operation_bu2_นุ่น04062025.xlsx"
