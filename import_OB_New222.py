        ...
        try:
            ...
            try:
                existing_po = models.execute_kw(
                    db, uid, password, 'purchase.order', 'search_count',
                    [[['name', '=', po_name]]]
                )
                if existing_po > 0:
                    print(f"Skipping PO {po_name} - Document already exists in the system")
                    continue
            except Exception as e:
                print(f"Error checking existing PO {po_name}: {e}")
                if not ensure_connection():
                    continue
                continue
